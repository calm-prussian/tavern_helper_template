<template>
  <section class="quick-grid">
    <article class="quick-card">
      <div class="card-head">
        <span>⛽ 油量</span>
        <span :class="['emphasis', { warn: fuel_percent <= 20 }]">{{ fuel_text }}</span>
      </div>
      <div class="progress-track">
        <div class="progress-fill fuel" :class="{ warn: fuel_percent <= 20 }" :style="{ width: `${fuel_percent}%` }"></div>
      </div>
      <div class="hint">容量 {{ store.data.车辆.组件.油箱.容量L }}L</div>
    </article>
    ...
    <article class="quick-card">
      ...
      <div class="fatigue-line">{{ fatigue_hint }}</div>
      <div class="hint">高与危险状态下应尽快寻找休息点</div>
    </article>
    ...
    <article class="quick-card">
      ...
      <div class="speed-line" :class="{ overspeed: is_overspeed }">{{ speed_text }}</div>
      <div class="hint">限速 {{ limit_text }}</div>
    </article>
